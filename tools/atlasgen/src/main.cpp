...
static uint8_t g_flattenFlags = gltfio::AssetPipeline::FILTER_TRIANGLES;
static bool g_flattenOnly = false;
...
Options:
   ...
   --flatten, -f
       Flatten the asset but do not parameterize it.
...
static int handleArguments(int argc, char* argv[]) {
    static constexpr const char* OPTSTR = "hLdf";
    ...
    while ((opt = getopt_long(argc, argv, OPTSTR, OPTIONS, &optionIndex)) >= 0) {
        ...
        switch (opt) {
            ...
            case 'L':
                ...
                exit(0);
            case 'd':
                g_flattenFlags |= gltfio::AssetPipeline::DISCARD_TEXTURES;
                break;
            case 'f':
                g_flattenOnly = true;
                break;
...
int main(int argc, char* argv[]) {
    ...
    gltfio::AssetPipeline::AssetHandle asset = pipeline.load(inputPath);
    if (!asset) {
    ...
    if (!asset) {
        std::cerr << "Unable to flatten " << inputPath << std::endl;
        exit(1);
    }
    if (!g_flattenOnly) {
        asset = pipeline.parameterize(asset);
        if (!asset) {
            std::cerr << "Unable to parameterize " << inputPath << std::endl;
            exit(1);
        }
    }
